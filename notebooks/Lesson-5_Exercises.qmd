...
```{r}
estadisticos_1 <- clase_1 |> 
  summarise(media_1 = mean(tiempo), n_1 = n(), varianza_1 = var(tiempo))

estadisticos_2 <- clase_2 |> 
  summarise(media_2 = mean(tiempo), n_2 = n(), varianza_2 = var(tiempo))

media_clase1 <- estadisticos_1 |> pull(media_1)
n_clase1 <- estadisticos_1 |>  pull(n_1)
varianza_clase1 <- estadisticos_1 |>  pull(varianza_1)

media_clase2 <- estadisticos_2 |> pull(media_2)
n_clase2 <- estadisticos_2 |> pull(n_2)
varianza_clase2 <- estadisticos_2 |> pull(varianza_2)

MU_POST1 <- (varianza_clase1 * MU_PREVIA + n_clase1 * SIGMA2_PREVIA * media_clase1 /
               (varianza_clase1 + n_clase1 * SIGMA2_PREVIA)) |> round(2)

MU_POST2 <- (varianza_clase2 * MU_PREVIA + n_clase2 * SIGMA2_PREVIA * media_clase2 /
               (varianza_clase2 + n_clase2 * SIGMA2_PREVIA)) |> round(2)
...
SIGMA2_POST1 <- (varianza_clase1 * SIGMA2_PREVIA / (varianza_clase1 + n_clase1 *SIGMA2_PREVIA))
SIGMA2_POST2 <- varianza_clase1 * SIGMA2_PREVIA / (varianza_clase2 + n_clase2 * SIGMA2_PREVIA)

SD_POST1 <- sqrt(SIGMA2_POST1)
SD_POST2 <- sqrt (SIGMA2_POST2)

MC1_500 <- rnorm(500, mean = MU_POST1, sd = SD_POST1)
MC2_500 <- rnorm(500, mean = MU_POST2, sd = SD_POST2)
```
...
MU1_MC500 <- mean(MC1_500)
MU2_MC500 <- mean(MC2_500)

VAR1_MC500 <- var(MC1_500)
VAR2_MC500 <- var(MC2_500)
...
El error estándar de Monte Carlo es $\sqrt{\hat{\sigma^2} / S}$

```{r}
SE1_MC500 <- sqrt(VAR1_MC500/length(MC1_500))
SE2_MC500 <- sqrt(VAR2_MC500/length(MC2_500))

IC1MC_500 <- c(MU1_MC500 - 1.96 * SE1_MC500, MU1_MC500 + 1.96 * SE1_MC500)
IC2MC_500 <- c(MU2_MC500 - 1.96 * SE2_MC500, MU2_MC500 + 1.96 * SE2_MC500)
...
```{r}
S_Muestra1 <- VAR1_MC500 / SE1_MC500^2
S_Muestra2 <- VAR2_MC500 /SE2_MC500^2 
```
