...
El valor máximo dependerá de los parámetros, pero será igual a $f(x) =+\infty$
:::
...
Para $f(x) = +\infty$
Este sería la moda, que es el más probable que puede asumir la variable aleatoria.
:::
...
El valor esperado en una distribución normal sería:
$$
Valor.Esperado = E(X) = \int_{-\infty}^{+\infty} x * f (x)dx 
$$
Dado que $X \sim N(\mu, \sigma)$ la media se conocería previamente, pero,
si se quiere conocer se puede estimar desde la distribución binomial $X \sim Bin(n, p)$.
En esta caso, $\mu = np$ 
:::
...
simétrica al igual que una distribución uniforme $DU(0, 1)$
:::
...
Con parámetros $Beta(\alpha = 2, \beta = 7)$ se ve que la densidad se inclina
hacia la izquierda, en donde la mayoría de valores se aproximan al límite inferior.
Sin embargo, mientras más aumenta alfa y toma un valor igual a beta
$Beta(\alpha = 7, \beta = 7)$ la distribución toma la forma de una distribución normal estandarizada, es decir:
$N(0, 1)$
:::
...
Para $Beta(1.5, 1.8)$ se observa que la gráfica comienza a parecerse a una distribución normal poco a poco 
...
beta |> ggplot(mapping = aes(x = theta, y = densidad)) + 
  geom_line(color = color_defecto) + ylim(0, max(beta$densidad))
```
En este caso se tiene que $\theta$ también está asumiendo una distribución uniforme $\theta \sim DU(0, 1)$ 
porque es una densidad simétrica e invariable de 0 a 1. 
:::
...
$$
L(\theta|y) = f(y|\theta) = \frac{n}{y} * \theta^y * (1-\theta)^{n-y}
$$
:::
